...
func expressionP() -> Parser<Exp> { // DO NOT MODIFY THIS LINE
    ...
    // on the right track.
	let integerParser: Parser<Exp> = mapP(numP(),
							{ input in Exp.integer(input) } 
	let temp: Parser<> = andP(tokenP(Token.leftParen),
					andP(tokenP(Token.plus), 
						andP(expressionP(), 
							andP(expressionP(),
								tokenP(Token.rightParen))))) 
	let plusParser: Parser<Exp> = mapP(temp,
					{ input in 
					let ( _, ( _ , (e1, (e2, _ )))) = input
					return Exp.plus(e1, e2) }
	return orP(orP (plusParser, integerParser),
		parseFooBar())
		
type Parser<Exp> = (List<Token>) -> ParseResult<Exp>
    func integerParser() -> (List<Token>) -> ParseResult<Exp> {
    return
     { tokens in	
    switch numP() (tokens)
        { 
        case .success (let rest, let value):
	        return ParseResult.success(rest, Exp.integer(value))
		case.failure (let msg):
			return ParseResult.failure(msg)
		}
	}
}
func integerPlusExpr() -> Parser<Exp> 
{
let parser = andP(integerParser(),
			andP(plusP(),
				parseExpr()))
}
func parseExpr() -> Parser<Exp> {
	return orP(integerPlusExpr(), integerParser())
