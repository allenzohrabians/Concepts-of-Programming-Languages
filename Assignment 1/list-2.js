...
class Cons extends List {
	...
	append(list) {
		return new Cons(this.h, this.t.append(list));
	}
	contains(number) {
		if (this.h == number) {
			return true;
		} else {
			return this.t.contains(number);
		}
	}
	length() {
	...
	init() {
		if (this.t instanceof Cons) {
			return new Cons(this.h, this.t.init());
		} else {
			return new Nil();
		}
	}
...
class Nil extends List {
	...
	append(list) {
		if (list instanceof Cons) {
			return new Cons(list.head(), this.append(list.tail()));
		} else {
			return new Nil();
		}
	}
...
function test1() {
	var c = new Cons(5, new Cons(6, new Nil()));
	process.stdout.write(c.toString() + "\n");
	process.stdout.write(c.join("~~") + "\n");
	process.stdout.write(c.length() + "\n");
	process.stdout.write(c.contains(5) + "\n");
	process.stdout.write(c.contains(6) + "\n");
	process.stdout.write(c.contains(7) + "\n");
	var d = new Cons(7, new Cons(8, new Cons(9, new Nil())));
	var cd = c.append(d);
	process.stdout.write(cd.toString() + "\n");
	var dc = d.append(c);
	process.stdout.write(dc.toString() + "\n");
	var n = new Nil();
	process.stdout.write(n.toString() + "\n");
	var nn = n.append(c);
	process.stdout.write(nn.toString() + "\n");
	var o = new Cons(5);
	process.stdout.write(o.toString() + "\n");
	var t = new Cons();
	process.stdout.write(t.toString() + "\n");
	var parent = Object.getPrototypeOf(c);
	process.stdout.write(parent.toString() + ", " + parent.hasOwnProperty("toString") + "\n");
	var grandparent = Object.getPrototypeOf(parent);
	process.stdout.write(grandparent.toString() + ", " + grandparent.hasOwnProperty("toString") + "\n");
}
